...
function fillTestParams(testParams, variables) {
  var ret = {};
  for (var key in testParams) {
    ret[key] = fillTestParam(testParams[key], variables);
  }
  return ret;
};

function parseParameters(testParams, methodParams, variables) {
  ...
  testParams = fillTestParams(testParams, variables);
...
function parseTest(host, uri, method, methodParams, test, variables) {
  //guard
  if (!test.request) test.request = {};
  var parameters = parseParameters(test.request.parameters, methodParams, variables);
    ...
    for (var method in path) {
      ...
      if (pathTests) {
        ...
        pathTests.forEach(function (test) {
          testSet.push(parseTest(host, uri, method, methodParams, test, variables));
        });
      }
